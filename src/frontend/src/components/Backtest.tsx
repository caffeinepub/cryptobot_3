...
function runSimulation(candles: Candle[]): BacktestResults {
  ...
  for (let i = START_INDEX; i < candles.length; i++) {
    ...
    // Entry check — simple momentum breakout
    if (i > START_INDEX + 5) {
      const prev5High = Math.max(...closes.slice(i - 5, i));
...
async function fetchAllCandles(
  ...
  const INTERVAL_MS = 5 * 60 * 1000; // 5m
  const BATCH = 1000;
  ...
  while (startTime < now) {
    ...
    const url = `https://api.binance.com/api/v3/klines?symbol=${symbol}&interval=5m&limit=${BATCH}&startTime=${startTime}`;
    ...
    <div className="space-y-6" data-ocid="backtest.panel">
      ...
      <div>
        ...
        <p className="mt-0.5 text-sm text-muted-foreground">
          Simulate the strategy on 1 year of {pairLabel} 5m historical data
        </p>
      </div>
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
              ...
              <StatCard
                ...
                sub={`≈ ${((results.avgDuration * 5) / 60).toFixed(1)} hours`}
                color="neutral"
              />
            </div>
      ...
      {status === "idle" && (
        ...
        >
          ...
          <div className="mt-1 text-xs text-muted-foreground">
            ...
            fetch 1 year of {pairLabel} 5m candles and simulate the strategy.
          </div>
        </motion.div>
      )}
    </div>
